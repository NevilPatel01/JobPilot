    ...
    <div>
      ...
      <div className="glass-panel overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {letters.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">No cover letters yet</td></tr>
            )}
            {letters.map((l) => (
              <tr key={l.id} className="border-b border-border/50 hover:bg-muted/30">
                ...
                <td className="px-4 py-3 text-muted-foreground">{formatDate(l.updated_at)}</td>
